...
###圣杯布局和双飞翼布局
- 圣杯布局  
两边固定宽度，中间自适应，dom结构必须先写中间列部分，可以优先加载  
步骤：  
1. 三个部分都设置左浮动，设置middle的宽度100%（实现中间列内容自适应），此时，left和right会跳到下一行  
2. 设置margin-left为负值，让left和right部分回到与middle同一行  
3. 设置父容器的padding-left和padding-right，让左右两边留出间隙  
4. 设置相对定位，让left和right部分移动到两边  
缺点：  
1. middle部分的最小宽度不能小于left部分的宽度，否则left部分会掉到下一行  
2. 如果其中一列内容高度拉长，其他两列的背景并不会自动填充  
- 双飞翼布局  
在圣杯布局基础上进行一步优化，解决了圣杯布局错乱问题，实现了内容与布局的分离。而且任何一栏都可以是最高栏，不会出问题  
步骤：  
1. 三个部分都设置左浮动，设置middle的宽度100%（实现中间列内容自适应），此时，left和right会跳到下一行  
2. 设置margin-left为负值，让left和right部分回到与middle同一行  
3. middle部分增加一个内层div，并设置margin：0 200px  
缺点：  
多加一层dom节点 增加渲染树生成的计算量  
- 两者对比  
...
- 利用等高布局解决高度不一致时不会自动填充的问题  
等高布局是指子元素在父元素中高度相等的布局方式。
1. 利用正padding+负margin  
因为背景是在 padding 区域显示的，设置一个大数值的 padding-bottom，再设置相同数值的负的 margin-bottom，并在所有列外面加上一个容器，并设置 overflow:hidden 把溢出背景切掉。这种可能实现多列等高布局，并且也能实现列与列之间分隔线效果，结构简单，兼容所有浏览器。
